...
export function BudgetContent() {
  const supabase = createBrowserClient()
  const now = new Date()
  const [year,setYear]=useState(now.getFullYear()); const [selMonth,setSelMonth]=useState(now.getMonth()+1)
  const [bm,setBm]=useState<Record<string,Record<number,number>>>({}); const [am,setAm]=useState<Record<string,Record<number,number>>>({})
  const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(false)
  const [dirty,setDirty]=useState<Set<string>>(new Set()); const [msg,setMsg]=useState('')

  const load=useCallback(async()=>{
    setLoading(true)
    const [bRes,eRes]=await Promise.all([
      supabase.from('budgets').select('*').eq('fiscal_year',year).neq('month',0),
      supabase.from('expenses').select('category,expense_date,amount_jpy').gte('expense_date',`${year}-01-01`).lte('expense_date',`${year}-12-31`)
    ])
    const newBm:Record<string,Record<number,number>>={}
    for(const b of (bRes.data??[]) as Budget[]){if(!newBm[b.category])newBm[b.category]={};newBm[b.category][b.month]=b.amount_jpy}
    setBm(newBm)
    const newAm:Record<string,Record<number,number>>={}
    for(const e of (eRes.data??[])){const m=parseInt(e.expense_date.slice(5,7));if(!newAm[e.category])newAm[e.category]={};newAm[e.category][m]=(newAm[e.category][m]??0)+e.amount_jpy}
    setAm(newAm); setLoading(false)
  },[year])
  useEffect(()=>{load()},[load])

  function updateCell(cat:string,m:number,val:string){
    const jpy=val===''?0:parseFloat(val)||0
    setBm(p=>({...p,[cat]:{...p[cat],[m]:jpy}})); setDirty(p=>new Set(p).add(`${cat}__${m}`))
  }
  ...
  async function save(){
    ...
    for(const key of dirty){
      ...
      ups.push({fiscal_year:year,month:m,category:cat,amount_thb:(bm[cat]?.[m]??0)/RATE,amount_jpy:bm[cat]?.[m]??0,updated_by:user?.id,updated_at:new Date().toISOString()})
    }
    await supabase.from('budgets').upsert(ups,{onConflict:'fiscal_year,month,category'})
    setDirty(new Set()); setSaving(false); setMsg(`${ups.length}件保存しました`); setTimeout(()=>setMsg(''),3000); load()
  }
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-bold text-slate-900">予算管理</h1><p className="text-sm text-slate-500 mt-0.5">セルをクリックして直接編集できます</p></div>
        <div className="flex items-center gap-2">
          <Select value={String(year)} onValueChange={v=>setYear(Number(v))}><SelectTrigger className="w-24 h-8 text-sm"><SelectValue/></SelectTrigger><SelectContent>{[now.getFullYear()-1,now.getFullYear(),now.getFullYear()+1].map(y=><SelectItem key={y} value={String(y)}>{y}年度</SelectItem>)}</SelectContent></Select>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[{l:`${year}年度 予算総額`,v:`¥${Math.round(totalB/10000)}万`},{l:'実績累計',v:`¥${Math.round(totalA/10000)}万`},{l:'消化率',v:`${totalB>0?Math.round(totalA/totalB*100):0}%`,w:totalA>totalB*0.9},{l:'残予算',v:`¥${Math.round(Math.max(totalB-totalA,0)/10000)}万`,w:totalA>totalB}].map(c=>(
          <Card key={c.l}><CardContent className="pt-4 pb-3"><p className="text-xs text-slate-500 mb-1">{c.l}</p><p className={`text-xl font-bold ${c.w?'text-red-600':''}`}>{c.v}</p></CardContent></Card>
        ))}
      </div>
      <Card><CardHeader className="pb-3"><CardTitle className="text-sm font-medium">月別 消化状況</CardTitle></CardHeader>
        <CardContent><div className="grid grid-cols-12 gap-1.5">
          {MONTHS.map(m=>{
            const b=CATEGORIES.reduce((s,c)=>s+(bm[c]?.[m]??0),0); const a=CATEGORIES.reduce((s,c)=>s+(am[c]?.[m]??0),0)
            const pct=b>0?Math.min(a/b*100,100):0; const isOver=a>b&&b>0; const isSel=m===selMonth
            return (<button key={m} onClick={()=>setSelMonth(m)} className={`rounded-xl border p-2 text-left transition-all ${isSel?'border-blue-500 bg-blue-50':'border-slate-200 hover:bg-slate-50'}`}>
              <p className={`text-[10px] font-medium mb-1.5 ${isSel?'text-blue-600':'text-slate-500'}`}>{m}月</p>
              <div style={{height:28,display:'flex',alignItems:'flex-end'}}><div style={{width:'100%',height:`${Math.max(pct,3)}%`,background:isOver?'#dc2626':pct>75?'#d97706':'#16a34a',borderRadius:2}}/></div>
      ...
      <Card><CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium">{year}年 {selMonth}月 — カテゴリ別</CardTitle>
      ...
      <CardContent>
        <table className="w-full text-sm">
          <thead><tr className="border-b"><th className="text-left py-2 pr-4 text-xs font-medium text-slate-500 w-28">カテゴリ</th><th className="text-right py-2 px-3 text-xs font-medium text-slate-500 w-40">予算（円）</th><th className="text-right py-2 px-3 text-xs font-medium text-slate-500 w-28">実績（円）</th><th className="text-right py-2 px-3 text-xs font-medium text-slate-500 w-28">差異（円）</th><th className="text-right py-2 pl-3 text-xs font-medium text-slate-500 w-20">消化率</th></tr></thead>
          <tbody>{CATEGORIES.map(cat=>{
            const b=bm[cat]?.[selMonth]??0; const a=am[cat]?.[selMonth]??0; const v=a-b; const rate=b>0?Math.round(a/b*100):null
            return (<tr key={cat} className="border-b hover:bg-slate-50">
              <td className="py-3 pr-4 font-medium text-sm">{cat}</td>
              <td className="py-3 px-3 text-right">
                <div className="flex items-center justify-end gap-1"><span className="text-xs text-slate-400">¥</span>
                  <input type="number" value={b===0?'':Math.round(b)} onChange={e=>updateCell(cat,selMonth,e.target.value)} placeholder="0" className="w-28 text-right text-sm bg-transparent border-0 border-b border-slate-200 focus:border-blue-500 focus:outline-none py-0.5 px-1"/></div>
              </td>
              <td className="py-3 px-3 text-right text-sm">{a>0?`¥${Math.round(a).toLocaleString()}`:'—'}</td>
              <td className={`py-3 px-3 text-right text-sm ${pctColor(a,b)}`}>{b===0?'—':(v>0?'+':'')+`¥${Math.round(v).toLocaleString()}`}</td>
              <td className="py-3 pl-3 text-right">{rate!==null?<div><span className={`text-xs font-semibold ${pctColor(a,b)}`}>{rate}%</span><div style={{height:3,background:'#e2e8f0',borderRadius:2,marginTop:3}}><div style={{width:`${Math.min(rate,100)}%`,height:'100%',borderRadius:2,background:rate>100?'#dc2626':rate>90?'#d97706':'#16a34a'}}/></div></div>:<span className="text-xs text-slate-400">—</span>}</td>
            </tr>)
          })}</tbody>
        </table>
        <p className="text-xs text-slate-400 mt-3">予算額を入力→「保存する」で確定。為替: 1THB = {RATE}円</p>
      </CardContent></Card>
    </div>
  )
}
